...
void dyncall_new(char const* _cname, char const* mname, Request& req, Response& res) {
  char const* cname = _cname;
  ControllerActionMapper* cam;
  try {
    cam = &(controller_mapper.at(cname));
...
void handle_request(raw_request const* req) {
  puts(">>handle_request<<");
  ...
  split(query_pairs, std::string(req->query, req->query_len), "&");
  ...
  std::string* method_s = new std::string(req->method, req->method_len);
  ...
  std::string* fullpath = new std::string(req->uri, req->uri_len);
  ...
  if (!router.route(std::string(req->path, req->path_len), *method_s, request, response)) {
