...
void parseURLEncodedBody(struct wsgi_request *wsgi_req,
        ...
        case URLDecodeParserState::Normal: {
          ...
          if (amp == nullptr) {
            ...
            if (eq == nullptr) {
              ...
              name = decodeURLString(current, eq);
            ...
            if (eq == nullptr) {
              forms.emplace(std::make_pair(decodeURLString(current, amp), ""));
            } else {
              forms.emplace(std::make_pair(decodeURLString(current, eq),
                                           decodeURLString(eq+1, amp)));
            }
            current = amp+1;
        ...
        case URLDecodeParserState::PartialValue: {
          ...
          if (amp == nullptr) {
            ...
            if (!value_holder.empty()) {
              ...
              value = decodeURLString(value_holder.begin(),
                                      value_holder.end());
              forms.emplace(std::make_pair(std::move(name),
                                           std::move(value)));
            } else {
              forms.emplace(std::make_pair(std::move(name),
                                           decodeURLString(current, amp))
              );
            }
            state = URLDecodeParserState::Normal;
            current = amp+1;
        ...
        case URLDecodeParserState::PartialName: {
          ...
          if (amp == nullptr) {
            ...
            if (eq == nullptr) {
              ...
              name = decodeURLString(name_holder.begin(), name_holder.end());
              value_holder.assign(eq+1, end);
              state = URLDecodeParserState::PartialValue;
            }
            current = end;
            ...
            if (eq == nullptr) {
              name_holder += std::string{current, end};
              forms.emplace(
                std::make_pair(decodeURLString(name_holder.begin(),
                                               name_holder.end()),
                               "")
              );
              state = URLDecodeParserState::Normal;
  ...
  if (state == URLDecodeParserState::PartialName) {
    forms.emplace(std::make_pair(
        decodeURLString(name_holder.begin(),name_holder.end()),
        ""
    ));
  } else if (state == URLDecodeParserState::PartialValue) {
    forms.emplace(std::make_pair(
        name,
        decodeURLString(value_holder.begin(),value_holder.end())
    ));
  }
  //return ret;
}
