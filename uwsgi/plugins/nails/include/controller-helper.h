...
class BaseController {
 public:
  ...
  bool protect_from_forgery = true;
...
typedef std::unordered_map<factory_base*,BaseController const*>
ControllerFactorySingletonMapper;
extern ControllerFactorySingletonMapper controller_factory_singleton_mapper;
...
//router.add("/foo/:id", {"GET"}, make_func(&FooController::show), type_name(singleton_FooController));
 public:
  ...
  bool route(std::string const& _path, std::string const& _method, Request& req) {
    for (auto const& r : routes) {
      ...
      if (bcp == nullptr) {
        ...
        auto* mf = static_cast<func<Response(BaseController*,Request&)>*>(fun);
        (*mf)(std::move(bcp), req);
        delete bcp;
        return true;
      }
